...
class TextDiscriminator(nn.Module):
    def __init__(
        ...
        # 64x64 to 32x32
        self.conv3 = nn.Conv2d(64, 64 * 2, 4, 2, 1, bias=False)
        self.bn3 = nn.BatchNorm2d(64 * 2)
        self.relu3 = nn.LeakyReLU(0.2, inplace=True)

        # 32x32 to 16x16
        self.conv4 = nn.Conv2d(64 * 2, 64 * 4, 4, 2, 1, bias=False)
        self.bn4 = nn.BatchNorm2d(64 * 4)
        self.relu4 = nn.LeakyReLU(0.2, inplace=True)

        # 16x16 to 8x8
        self.conv5 = nn.Conv2d(64 * 4, 64 * 8, 4, 2, 1, bias=False)
 	...
 	self.adpmp = nn.AdaptiveMaxPool2d(1)

        # concatenate text along channel as features
	# set text = 6 for now (512+6 = 518) 
        self.conv7 = nn.Conv2d(518,64,kernel_size=(1,1))
 	self.bn7 = nn.BatchNorm2d(64)
        self.relu7 = nn.LeakyReLU(0.2, inplace=True)
 
        self.conv8 = nn.Conv2d(64,1,kernel_size=(1,1))
	...
	conv1 = self.conv1( input )
        ...
        adpmp = self.adpmp(relu6)
        ## concatenate text, 1 is for column wise concatenate

        pdb.set_trace()

        con = torch.cat(relu6, text, 1)

        conv7 = self.conv7(con)
        bn7 = self.bn6( tconv6 )
        relu7 = self.relu6(tbn6)

        conv8 = self.conv8(relu7)
        
        return torch.sigmoid(conv8) 
