## New Model Text Script Date: Oct 16th
## Author: Yang Gao
...
def read_text(pathlist):
    path_text = []
    for i in range(len(pathlist)): 
        text = pathlist[i][-11:-6]
        text = ['0' if x=='o' else x for x in text]
        path_text.append(''.join(text))
...
def get_text_loss(Ap, Bp, Anp, Bnp, criterion, cuda):
    labels_Ap = Variable(torch.ones( [Ap.size()[0], 1] ))
    labels_Bp = Variable(torch.ones([Bp.size()[0], 1]))
    labels_Apn = Variable(torch.zeros([Apn.size()[0], 1] ))
    labels_Bpn = Variable(torch.zeros([Bpn.size()[0], 1] ))

    if cuda:
        labels_Ap = labels_Ap.cuda()
        labels_Bp = labels_Bp.cuda()
        labels_Apn = labels_Apn.cuda()
        labels_Bpn = labels_Bpn.cuda()

    txt_loss = criterion( Ap, labels_Ap ) * 0.25 + criterion( Bp, labels_Bp ) * 0.25 + criterion( Apn, labels_Apn ) * 0.25 + criterion( Bpn, labels_Bpn ) * 0.25
...
def main():
    ...
    discriminator_T = TextDiscriminator(args.num_gpu, [])
    ...
    if cuda:
        ...
        discriminator_T = discriminator_T.cuda()
    ...
    if args.num_gpu > 1:
        ...
        discriminator_T = nn.DataParallel(discriminator_T, device_ids = range(args.num_gpu))
    ...
    txt_params =  discriminator_T.parameters()
    ...
    optim_txt = optim.Adam( txt_params, lr=args.learning_rate, betas=(0.5,0.999), weight_decay=0.00001)
    ...
    for epoch in range(epoch_size):
        ...
        for i in range(n_batches):
            ...
            if args.task_name =='spectrogram':
               ...
               text_A = read_text( A_path )
               text_B = read_text( B_path )
 	    ...
 	    ## Text Loss
	    # suppose to keep target
            pdb.set_trace()
            textP_A = TextDiscriminator(A, text_A) 
            textP_BA = TextDiscriminator(BA, text_B)
            textP_ABA = TextDiscriminator(ABA, text_A)
            textP_B = TextDiscriminator(B, text_B)
            textP_AB = TextDiscriminator(AB, text_A)
            textP_BAB = TextDiscriminator(BAB, text_B)
	    # suppose to not keep target
            textP_An = TextDiscriminator(A, text_B)
	    textP_BAn = TextDiscriminator(BA, text_A)
            textP_ABAn = TextDiscriminator(ABA, text_B)
            textP_Bn = TextDiscriminator(B, text_A)
            textP_ABn = TextDiscriminator(AB, text_B)
            textP_BABn = TextDiscriminator(BAB, text_A) 

            # pdb.set_trace()
            text_loss_real = get_text_loss(textP_A, textP_B, textP_An, textP_Bn, txt_criterion, cuda)
            text_loss_fake1 = get_text_loss(textP_AB, textP_BA, textP_ABn, textP_BAn, txt_criterion, cuda)
            text_loss_fake2 = get_text_loss(textP_ABA, textP_BAB, textP_ABAn, textP_BABn, txt_criterion, cuda)
            text_loss = text_loss_real + text_loss_fake1 + text_loss_fake2
...
#            recon_loss_B = 0.3*recon_loss_AB + 0.7*recon_loss_BAB
	    recon_loss_A = 3*recon_loss_BA + 7*recon_loss_ABA
	    recon_loss_B = 3*recon_loss_AB + 7*recon_loss_BAB 
	    ...
	    BAB_stl = discriminator_S(BAB)
            ...
            if args.model_arch == 'discogan':
                ...
                gen_loss = gen_loss_A_total + gen_loss_B_total
                dis_loss = dis_loss_A + dis_loss_B + stl_loss + text_loss
		...
		print "Text Loss:", as_np(txt_loss.mean())
		...
		log_txt_loss = np.concatenate([log_txt_loss, as_np(txt_loss.mean())])
    ...
    sio.savemat('log_txt_loss.mat', {'log_txt_loss':log_txt_loss})
